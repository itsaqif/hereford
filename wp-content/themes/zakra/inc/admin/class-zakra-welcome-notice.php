...
class Zakra_Welcome_Notice {
	...
	public function __construct() {
		...
		add_action( 'wp_loaded', array( $this, 'release_notice' ), 20 );
	...
	public function release_notice() {
		if ( ! get_option( 'zakra_admin_notice_release' ) ) {
			add_action( 'admin_notices', array( $this, 'release_notice_markup' ) ); // Show notice.
		}
	}
	...
	public function release_button_html() {
		$html = '<a class="btn-get-release button button-primary button-hero" style="color: #fff !important;" target="_blank" href="https://zakratheme.com/blog/zakra-3-0/">' . esc_html__( 'Learn More', 'zakra' ) . '</a>';
	 ...
	 * Show release notice.
	 */
	public function release_notice_markup() {
		$dismiss_url = wp_nonce_url(
			remove_query_arg( array( 'activated' ), add_query_arg( 'zakra-hide-notice', 'release' ) ),
			...
			<div class="zakra-message__content">
				...
				<div class="zakra-message__text">
					<h2 class="zakra-message__heading" style="font-weight: 500">
						<?php
						printf(
							esc_html__( '[IMPORTANT!] Announcement regarding Zakra 3.0 and Zakra Pro 2.0 Major Releases', 'zakra' ),
					...
					<div class="zakra-message__cta">
						<?php echo $this->release_button_html(); ?>
						<span class="plugin-install-notice"><?php esc_html_e( 'Please read the article by clicking the button for detailed instructions on safely updating to the major versions.', 'zakra' ); ?></span>
					</div>
				</div>
			</div>
	...
	public function hide_notices() {
		if ( isset( $_GET['zakra-hide-notice'] ) && isset( $_GET['_zakra_notice_nonce'] ) ) { // WPCS: input var ok.
			...
			} elseif ( 'release' === $_GET['zakra-hide-notice'] ) {
				update_option( 'zakra_admin_notice_release', 1 );
			} else {
